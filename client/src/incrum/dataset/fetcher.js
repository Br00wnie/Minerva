...
export const fetch = (request, payload, starterId, setState, useCache, onSuccess, onError, onProgress) => {
	...
	if (payloadSnapshot.state == IDLE || payloadSnapshot.state == ERROR) {
		...
		// check if middleware returns state with error
		...
		(new Promise((presolve, preject) => {
			...
			data => {	// run order - 2
				...
				// processedData contains actual loaded data processed with middleware.
...
// middleware in request has to get stack parameter to himself and working only with stack mutations
// each middleware has to return stack.next(stack) to call next middleware or return stack to break next middleware processing.
// middleware can mutate stack.payload or stack.response
