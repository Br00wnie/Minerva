...
const createServiceContainer = (store, container, containerDependencies) => {
  ...
  const service = new Proxy(holder, {
    get: (_holder, method, thisProxy) => {
      ...
      } else if (_holder.dependencies[method] !== undefined) {
...
const subscribe = (prop, store) => {
  ...
  idStack.push(id);
  const redraw = redrawStack.pop();
  redrawStack.push(redraw);
...
const buildStore = (obj, opts) => {
  ...
  const store = new Proxy(obj, {
    ...
    get: (initialStore, prop, storeInstance) => {
      ...
      if (!initialStore.__subscribtionStopped) subscribe(prop, initialStore);
...
const StoreCapsule = ({ propname, store, template }) => {
  ...
  return (
    <>
      {template && helpers.isFunction(template)
        ? template(innerstore[propname])
        : innerstore[propname]}
    </>
  );
};
    ...
    store
      ...
      .extend("capsulate", (initialStore, store) => {
        return (prop, template) => (
          <StoreCapsule propname={prop} store={store} template={template} />
        );
      })
...
export { useStore, buildStore, useRefreshTriggeredBy };
