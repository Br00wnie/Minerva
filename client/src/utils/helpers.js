...
export const helpers = {
  /**
   * Returns unique for script lifetime identifier.
   * @returns {string}
   */
  getId: () => "i" + Id++,
  ...
  /**
   * Checks the given value is function.
   * @param {any} obj Value to check for.
   * @returns {boolean}
   */
  ...
  /**
   * Checks if given argument is arrow function, something like 'const func = () => {}'.
   * @param {any} f Value to check
   * @returns {boolean}
   */
  isArrowFn: (f) =>
    typeof f === "function" &&
  ...
  /**
   * Determines if client is mobile device.
   * @returns {boolean}
   */
  isMobile: function () {
    return (function (a) {
  ...
  /**
   * Determines if given string contains valid Json.
   * @param {string} str Probably Json string
   * @returns {boolean}
   */
  ...
  /**
   * Determines if given value is integer type.
   * @param {any} a Value to check for
   * @returns {boolean}
   */
  isInt: (a) => {
  ...
  /**
   * Returns count of elements of array or keys (props) of object.
   * @param {Object|array} o Object or array to get count of items (props) from
   * @returns {int}
   */
  count: (o) => {
  ...
  /**
   * Return first element of array or value of first property of object.
   * @param {Object|array} o Object or array to get first item from
   * @returns {any}
   */
  first: (o) => {
  ...
  /**
   * Checks if given value exists in given object or array.
   * @param {any} value Value to find
   * @param {Object|array} array Array or object to check value exists in
   * @returns {boolean}
   */
  in_array: (value, array) => {
    ...
    if (helpers.isObject(array)) {
      ...
      for (let i in array) if (array[i] == value) return true;
    }
  ...
  /**
   * Finds given value in array and returns its index or -1 if nothing found.
   * @param {any} value Value to search in array
   * @param {array} array Array to find value in
   * @returns {int}
   */
  ...
  /**
   * Checks if key exists in given object
   * @param {string} key Key name to find
   * @param {Object} obj Object to find key in
   * @returns {boolean}
   */
  key_exists: (key, obj) =>
    helpers.isObject(obj) && Object.keys(obj).indexOf(key) !== -1,
  ...
  /**
   * Merges obj1 with key-values of obj2.
  ...
  /**
   * Compares to abstract values of any type.
   * @param {any} object1 First abstract to compare
   * @param {any} object2 Second abstract to compare
   * @returns {boolean}
   */
  isEqual: (object1, object2) => {
    if (helpers.isObject(object1) && helpers.isObject(object2)) {
      ...
      for (const key of keys1) {
        ...
        const areObjects =
          (helpers.isObject(val1) && helpers.isObject(val2)) ||
          (helpers.isArray(val1) && helpers.isArray(val2));

        if (
          (areObjects && !helpers.isEqual(val1, val2)) ||
      ...
      return true;
    } else if (helpers.isArray(object1) && helpers.isArray(object2)) {
  ...
  /**
   * Stops script processing for a <delay> seconds.
  ...
  /**
   * Detemines if given value is empty. Empty object is object without keys, empty array when its length is zero,
   * empty scalar is undefined or null, empty string when its length is zero.
   * @param {any} abstract
   * @returns {boolean}
   */
  empty: (abstract) => {
    if (abstract === undefined || abstract === null) return true;

    if (typeof abstract == "string" || helpers.isArray(abstract))
      return abstract.length === 0;
    else if (helpers.isObject(abstract))
      return Object.keys(abstract).length === 0;
...
export const foreach = (obj, callback) => {
  if (helpers.isArray(obj)) return obj.map(callback);
  else if (helpers.isObject(obj)) return Object.keys(obj).map(callback);
  else callback(obj);
};
...
/**
 * Creates a copy of given value to allow using object or arrays as new copy but
 * not a reference.
 * @param {any} data Any value to copy from (an object, array or ant scalar)
 * @returns {any}
 */
export const copy = (data) => {
  if (helpers.isObject(data)) {
    let obj = {};
    foreachKey(data, (key, value) => (obj[key] = copy(value)));
    return obj;
